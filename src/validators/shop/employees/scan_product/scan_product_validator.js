 ...
 */
module.exports.scan_product_validator = async (input) => {
  // define variables
  var errors = {};
  var required = ["employee_id", "shop_id", "barcode"];
    ...
    if (input.hasOwnProperty(key)) {
      ...
      switch (key) {
        ...
        case "barcode":
          if (value.length > 255) {
            errors[key].push(
              "Invalid barcode, Cannot greater than 255 characters."
            );
          } else if (value.length < 1) {
            errors[key].push("Invalid barcode, Cannot less than 1 character.");
          }
          break;
        default:
          break;
