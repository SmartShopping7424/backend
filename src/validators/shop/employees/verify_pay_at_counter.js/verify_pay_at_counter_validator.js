 ...
 */
module.exports.verify_pay_at_counter_validator = async (input) => {
  // define variables
  var errors = {};
  var required = ["employee_id", "shop_id", "payment_id"];
    ...
    if (input.hasOwnProperty(key)) {
      ...
      switch (key) {
        ...
        case "payment_id":
          if (value.length > 255) {
            errors[key].push(
              "Invalid payment_id, Cannot greater than 255 characters."
            );
          } else if (value.length < 1) {
            errors[key].push(
              "Invalid payment_id, Cannot less than 1 character."
            );
          }
          break;
        default:
